...
def run_test():
    ...
    # 0. 테스트 데이터 준비 (지방선거 사례 vs 정치 이슈 사례)
    data = [
        # {
        #     "news_id": 0,
    ...
    print("="*60)
    print(" [검증 시작] SimHash(중복 제거) vs SingleNewsClusterer(그룹핑+대표선정)")
    print("="*60)

    # 1단계: SimHash 테스트 (Near-Duplicate 제거용)
    # content_distance = 0
    # print(f"\n1단계: SimHash 테스트 (SIMHASH_BODY_DISTANCE Dist: {SIMHASH_BODY_DISTANCE})")
    sh_dedup = SimHashDeduplicator(title_distance=SIMHASH_TITLE_DISTANCE,
