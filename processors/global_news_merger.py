...
class GlobalNewsMerger:
    ...
    def group(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "title" not in df.columns:
            raise ValueError("GlobalNewsMerger requires 'title' column")

        content_col = "content" if "content" in df.columns else "content"
        if content_col not in df.columns:
            raise ValueError("GlobalNewsMerger requires 'content' or 'content' column")
