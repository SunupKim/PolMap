# python -m processors.news_cluster
...
class NewsCluster:
    ...
    def _save_similarity_debug_log(self, df: pd.DataFrame, keyword: str):
        ...
        if "pubDate" in debug_df.columns:
            debug_df["pubDate"] = pd.to_datetime(debug_df["pubDate"], errors="coerce").dt.strftime("%Y-%m-%d %H:%M:%S")
        ...
        # # 5. 표준 컬럼 순서 및 pubDate 포맷 강제 저장 (전체 df 기준)
        # from utils.dataframe_utils import standardize_and_save
        # from config import RAW_COLUMNS
        # standardize_and_save(df, path, columns=RAW_COLUMNS)
    ...
    data = [
        {
            ...
            "title": "李 대통령, '설탕세' 제안…"부담금으로 지역 의료 재투자"", "description": "설명1", 
            "link": "url1", "originallink": "ourl1", "content": "본문1"
        },
        {
            "news_id": "a2", "pubDate": "2026-01-28 10:00:01", "collected_at": "2026-01-28 10:01:00",
            "title": "이 "담배처럼 설탕부담금, 지역·공공의료 투자" 제안", "description": "설명2", 
            "link": "url2", "originallink": "ourl2", "content": "본문2"
        },
        {
            "news_id": "a3", "pubDate": "2026-01-28 10:00:02", "collected_at": "2026-01-28 10:01:00",
            "title": "이재명 대통령 호통에 '반값 생리대' 등장... 그러나", "description": "설명3", 
            "link": "url3", "originallink": "ourl3", "content": "본문3"
        }
    ]
    df = pd.DataFrame(data)
    cluster = NewsCluster(title_threshold=0.2, content_threshold=0.15)
