...
- 뉴스가 충분히 쌓이기 전에는 폴맵(좌·우 좌표화) 대신 인물 서술형 프로파일링을 먼저 한다.

- 입력은 인물 1명(예: 이언주), 출력은 뉴스 근거 기반의 강점·공백·근접자 서술이다.

- RAG로 해당 인물 관련 뉴스를 모으고, 이슈는 정치권 전체에서 먼저 도출한 뒤 인물을 그 위에 얹어 상대적으로 평가한다.

- 공백은 “없는 이슈”가 아니라 판에는 있는데 역할 기대 대비 중심성이 낮은 이슈로 정의한다.

- 경쟁자라는 단정은 피하고, **같은 이슈 공간에서 의미적으로 가까운 인물들을 ‘근접자’**로 제시한다.

- 판단은 수치/분포(이슈 클러스터별 중심성, 동시 등장, 임베딩 거리)로 하고, LLM은 요약·서술만 담당한다.

- 이 방식은 데이터가 적어도 가능하며, 이후 폴맵 좌표화로 자연스럽게 확장된다.
